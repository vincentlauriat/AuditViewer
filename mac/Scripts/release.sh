...
# Compte keychain de la clé EdDSA Sparkle (partagée entre les apps).
SPARKLE_ACCOUNT="${SPARKLE_ACCOUNT:-MarkdownViewer}"
GH_REPO="${GH_REPO:-vincentlauriat/AuditViewer}"

echo "→ xcodegen generate"
xcodegen generate >/dev/null
echo "→ xcodebuild Release"
xcodebuild -project AuditViewer.xcodeproj \
  -scheme AuditViewer -configuration Release \
  -derivedDataPath build CODE_SIGNING_ALLOWED=NO build 2>&1 | tail -5

APP="$ROOT/build/Build/Products/Release/AuditViewer.app"
[ -d "$APP" ] || { echo "✗ App non trouvée : $APP" >&2; exit 1; }

STAGING_DIR="$(mktemp -d)"
STAGING="$STAGING_DIR/AuditViewer.app"
...
ditto --norsrc --noextattr --noacl "$STAGING" "$DMG_LAYOUT/AuditViewer.app"
...
tell application "Finder"
    tell disk "$DMG_VOLNAME"
        ...
        set position of item "AuditViewer.app" of container window to {140, 200}
...
BUILD_NUMBER=$(/usr/libexec/PlistBuddy -c "Print :CFBundleVersion" \
  "$ROOT/build/Build/Products/Release/AuditViewer.app/Contents/Info.plist")
PUB_DATE=$(date -R)
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">
  <channel>
    ...
    <item>
      ...
      <enclosure
        url="https://github.com/$GH_REPO/releases/download/v$VERSION/AuditViewer-$VERSION.dmg"
